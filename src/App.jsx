...
export default function App() {
  ...
  const [resultPreviewUrl, setResultPreviewUrl] = useState(null); // URL of the rendered result (video with boxes burned in, or original for image)
  ...
  const handleClear = () => {
    setFile(null); setFilePreviewUrl(null); setResultPreviewUrl(null); setUploadedFileId(null);
  ...
  const handleSendMessage = async (text) => {
    ...
    try {
      ...
      if (data.detections && data.detections.length > 0) {
        ...
        // For video: backend returns a pre-rendered video URL with boxes burned in.
        // For image: use the original file URL (boxes are overlaid in the frontend).
        if (data.result_url) {
          setResultPreviewUrl(`http://localhost:8000${data.result_url}`);
        } else {
          setResultPreviewUrl(filePreviewUrl);
        }
    ...
    <div className="app-container">
      ...
      <main className="main-content">
        {mode === 'upload' ? (
          <>
            <UploadArea
              filePreviewUrl={resultPreviewUrl || filePreviewUrl}
              isVideo={isVideo}
